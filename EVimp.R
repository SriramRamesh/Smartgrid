getdsm<- function(stageNo,userNo){
  ...
  temp_index<-end/8
  if(userNo==1){start<-1}
  else{start<-temp_index*(userNo-1)+1}
...
while(i<=144){
  ...
  if(Stages$No_of_cars[i]>=1){
    ...
    while(loop_cars<=Stages$No_of_cars[i]){
      ...
      Contribution_User[i,temp_user]<-Contribution_User[i,temp_user]+(100*Stages$Error[i]/max(Stages$Error))+(100*(getdsm(i,temp_user)+getWind(i,temp_user)+getSolar(i,temp_user)))
...
# No_of_cars<0 => User with highest contribution factor can charge 
  ...
  while(loop_cars<=(-1*Stages$No_of_cars[i])){
    ...
    Contribution_User[i,temp_user]<-Contribution_User[i,temp_user]-(100*Stages$Error[i]/min(Stages$Error))+(100*(getdsm(i,temp_user)+getWind(i,temp_user)+getSolar(i,temp_user)))
